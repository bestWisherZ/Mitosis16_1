...
func TestProofs(t *testing.T) {
	trie, vals := randomTrie(150)
	root := trie.Hash()

	var valKeys [][]byte
	proof := memorydb.New()
	nodeProof := memorydb.New()
	blocktx := 1
	...
	for _, kv := range vals {
		...
		valKeys = append(valKeys, kv.k)
		trie.Prove(kv.k, 0, proof)
		blocktx--
	}

	PruneProofs(root, []byte{}, proof, nodeProof)

	trie2, ok, _ := VerifyProofs(root, nodeProof)

	for _, kv := range valKeys {
		val2, _ := trie2.TryGet(kv)
		val, _ := trie.TryGet(kv)
		fmt.Println(val2, val)
		if !bytes.Equal(val, val2) {
			t.Fatalf("failed valnode")
		}

	}
	if !ok {
		t.Fatalf("failed to verify proofs")
	}

}

func TestProofs2(t *testing.T) {
	trie, vals := randomTrie(2)
	var accouts [][]byte
	for _, kv := range vals {
		accouts = append(accouts, kv.k)
		break
	}
	//accouts=append(accouts,randBytes(32))
	//accouts=append(accouts,randBytes(32))
	root := trie.Hash()
	f := func(keys [][]byte, fromLevel uint, proofDb ethdb.KeyValueWriter) error {
		proof := memorydb.New()
		for _, key := range keys {
			trie.Prove(key, fromLevel, proof)
		}
		var key []byte
		PruneProofs(trie.Hash(), key, proof, proofDb)
		return nil
	}

	pathProof := memorydb.New()
	valProof := memorydb.New()
	for _, acc := range accouts {
		val, err := trie.TryGet(acc)
		if err == nil && val != nil {
			valProof.Put(acc, val)
		}
	}
	err := f(accouts, 0, pathProof)
	if err != nil {
		t.Fatalf("1111111")
	}

	var pathKeys, valKeys [][]byte
	it := pathProof.NewIterator(nil, nil)
	for it.Next() {
		pathKeys = append(pathKeys, it.Key())
	}
	it = valProof.NewIterator(nil, nil)
	for it.Next() {
		valKeys = append(valKeys, it.Key())
	}
	trie, ok, _ := VerifyProofs(root, valProof)
