...
func (c *committer) commit(n node, db *Database) (node, error) {
	...
	case *shortNode:
		...
		collapsed.Key = binaryToCompact(cn.Key)
...
func (c *committer) commitChildren(n *fullNode, db *Database) ([3]node, error) {
	var children [3]node
	for i := 0; i < 2; i++ {
		child := n.Children[i]
		if child == nil {
	...
	// For the 3th child, it's possible the type is valuenode.
	if n.Children[2] != nil {
	...
	for item := range c.leafCh {
		...
		if c.onleaf != nil {
			...
			case *fullNode:
				...
				if n.Children[2] != nil {
					c.onleaf(nil, n.Children[2].(valueNode), hash)
				}
			}
		}
	}
